...
CREATE OR REPLACE FORCE VIEW vw_qos_region AS
  ...
  FROM vw_period_license INNER JOIN qos_region ON vw_period_license.period_license_id = qos_region.period_license_id;
...
BEGIN
	...
	FROM period_license INNER JOIN client_license ON period_license.client_license_id = client_license.client_license_id
	WHERE (period_license.IS_COMPLIANT = '0') AND (client_license.client_id = clientid);
	IF(ncpl is null)THEN ncpl := 0; END IF;

	IF(insp = 0) AND (notc = 0) AND (notl = 0) AND (ncpl = 0) THEN
...
BEGIN
	...
	SELECT period_license_id, client_license_id, is_complied INTO lcid, clid, iscom
...
CREATE OR REPLACE TRIGGER upd_qos_compliance BEFORE UPDATE ON qos_compliance FOR EACH ROW 
DECLARE
	PRAGMA AUTONOMOUS_TRANSACTION;
	ctype 	varchar2(5);
	tvalue	FLOAT(63);
BEGIN
	...
	if updating then 
		SELECT target_operator, target_value INTO ctype, tvalue
		FROM qos_factor
		WHERE qos_factor_id = :NEW.qos_factor_id;

		if (ctype = '>=') then
			if (:NEW.actual_cck_value >= tvalue) then
				:NEW.is_complied := '1';
			else
				:NEW.is_complied := '0';
			end if;
