...
ALTER TABLE passengers ADD COLUMN portal_id integer references portal;
CREATE INDEX passengers_portal_id ON passengers (portal_id);
...
UPDATE passengers SET portal_id = 2 WHERE entity_id IN(select entity_id from entitys where function_role = 'broker');
UPDATE passengers SET portal_id = 1 WHERE entity_id IN(select entity_id from entitys where function_role != 'broker');
