...
CREATE VIEW vw_loans AS
	SELECT
		...
		entitys.entity_id, entitys.entity_name, loans.org_id, loans.loan_id, loans.principle, loans.interest, loans.monthly_repayment, loans.reducing_balance, 
...
CREATE VIEW vw_loan_payments AS
	SELECT	vw_loans.currency_id, vw_loans.currency_name, vw_loans.currency_symbol,
		...
		get_loan_period(vw_loans.principle, vw_loans.interest, generate_series(1, vw_loans.repayment_period), repayment_amount) as loan_balance, 
		
		(get_loan_period(vw_loans.principle, vw_loans.interest, generate_series(1, vw_loans.repayment_period) - 1, repayment_amount) * (vw_loans.interest/1200)) as loan_intrest 
	FROM vw_loans
