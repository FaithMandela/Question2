...
CREATE VIEW qlocation_stats AS
	(SELECT 1 as statid, text 'Opened Applications' AS "narrative", count(qstudentid) AS studentcount, 
		quarterid, levellocationname, sublevelname
	FROM qstudentview GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 2, text 'Paid Full Fees' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname
		FROM studentquarterview WHERE (finalbalance >= (-2000)) AND (finaceapproval = true) 
		GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 3, text 'Within Allowed Balance' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname
		FROM studentquarterview WHERE (finalbalance < (-2000)) AND (finalbalance >= ((-1) * feesline)) AND (finaceapproval = true)
		GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 4, text 'Above Allowed Balance' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname
		FROM studentquarterview WHERE (finalbalance >= ((-1) * feesline)) AND (finaceapproval = true)
		GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 5, text 'Below Allowed Balance' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname
		FROM studentquarterview WHERE (finalbalance < ((-1) * feesline))
		GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 6, text 'Financially Approved' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname 
		FROM qstudentview WHERE (finaceapproval = true)
		GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 7, text 'Approved and Below Allowed Balance' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname
		FROM studentquarterview WHERE (finalbalance < ((-1) * feesline) AND (finaceapproval = true)) 
		GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 8, text 'Not Approved and Above Allowed Balance' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname
		FROM studentquarterview WHERE (finalbalance >= ((-1) * feesline) AND (finaceapproval = false)) 
		GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 9, text 'Closed Applications' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname 
		FROM qstudentview
		WHERE (finalised = true) GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 10, text 'Closed and not Finacially approved' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname 
		FROM qstudentview WHERE (finalised = true) AND (finaceapproval = false) 
		GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 11, text 'Printed Applications' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname
		FROM qstudentview WHERE (printed = true) 
		GROUP BY quarterid, levellocationname, sublevelname)
	UNION
	(SELECT 12, text 'Fully Registered' AS "narrative", count(qstudentid), quarterid, levellocationname, sublevelname
		FROM qstudentview WHERE (approved = true) 
		GROUP BY quarterid, levellocationname, sublevelname);
