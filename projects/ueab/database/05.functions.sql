...
BEGIN
	...
	IF(v_qgradeid is null)THEN
		...
		WHERE (qcourseid = CAST($1 as int)) AND (lecture_gradeid <> 'NG');
...
CREATE OR REPLACE FUNCTION get_degreelevelid(varchar(16)) RETURNS varchar(16) AS $$
	SELECT sublevels.degreelevelid
	FROM sublevels INNER JOIN studentdegrees ON sublevels.sublevelid = studentdegrees.sublevelid 
	WHERE (studentdegrees.studentdegreeid = getstudentdegreeid($1));
$$ LANGUAGE SQL;
