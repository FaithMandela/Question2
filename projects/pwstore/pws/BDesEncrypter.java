...
public class BDesEncrypter {
	...
	public void encrypt(String fileString, String outFileName, boolean isText) {
		try {
			// convert String into InputStream
			InputStream is = new ByteArrayInputStream(fileString.getBytes());

			// Encrypt
			encrypt(is, new FileOutputStream(outFileName));
		} catch (Exception ex) {
			System.out.println("Error : " + ex);
		}
	}
	...
	public String decrypt(String inFileName, boolean needText) {
		InputStream in = decrypt(inFileName);
		String dataOut = null;
		if(in != null) dataOut = new Scanner(in, "UTF-8").useDelimiter("\\A").next();
	...
	public InputStream decrypt(String inFileName) {
		...
		try {
			...
			if(fl.exists()) in = decrypt(new FileInputStream(inFileName));
