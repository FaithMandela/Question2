...
CREATE VIEW vw_sms AS
	SELECT folders.folder_id, folders.folder_name, sms.sms_id, sms.sms_number, 
		...
		vw_entity_address.entity_name, vw_entity_address.mobile
	FROM sms INNER JOIN folders ON sms.folder_id = folders.folder_id
		LEFT JOIN vw_entity_address ON sms.sms_number = vw_entity_address.mobile;
...
CREATE VIEW vw_sms_address AS
	SELECT vw_entity_address.entity_name, vw_entity_address.mobile,
		sms_address.sms_address_id, sms_address.sms_id, sms_address.narrative
	FROM vw_entity_address INNER JOIN sms_address ON vw_entity_address.address_id = sms_address.address_id;
...
BEGIN
	IF((substr(NEW.sms_number, 1, 1) = '+') AND (NEW.folder_id = '3')) THEN
		NEW.folder_id := 4;
	END IF;
