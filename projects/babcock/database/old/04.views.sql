CREATE VIEW applicationview AS 
	SELECT applications.applicationid, applications.email, applications.firstpasswd, applications.applicationdate,
		applications.surname, applications.firstname, applications.othernames, applications.confirmationno,
		applications.receiptnumber, applications.purchasecentre, applications.amount, applications.paid,
		applications.appoved, applications.closed, 'Babcock Application Responce'::varchar AS emailsubject,
		(CASE WHEN applications.paid = false THEN 
		'<a href="paymentApplicant.jsp?confirmationno='|| applications.confirmationno
		|| '&amount=' || applications.amount || '&receiptno=' || applications.receiptnumber
		...
		(CASE WHEN applications.paid = false THEN 
		'<a href="VPC_JSP_3P_app.jsp?amount=' || applications.amount || '&transId=' || applications.applicationid
		|| '" target="_blank"><IMG SRC="images/visa.jpeg" WIDTH=120 HEIGHT=24 ALT=""></a>'
...
CREATE VIEW registrymarkview AS
	SELECT registrationview.registrationid, registrationview.fullname, subjects.subjectid, subjects.subjectname, 
...
CREATE VIEW sublevelview AS
	SELECT degreelevels.degreelevelid, degreelevels.degreelevelname,
		...
		sublevels.sublevelid, sublevels.sublevelname, sublevels.details
...
CREATE VIEW instructorview AS
	SELECT departmentview.schoolid, departmentview.schoolname, departmentview.departmentid, departmentview.departmentname,
		...
		instructors.majoradvisor, instructors.headofdepartment, instructors.headoffaculty, instructors.email
...
CREATE VIEW studentmajorview AS 
	SELECT studentdegreeview.religionid, studentdegreeview.religionname, studentdegreeview.denominationid, studentdegreeview.denominationname,
		...
		studentmajors.studentmajorid, studentmajors.major, studentmajors.nondegree, studentmajors.premajor, studentmajors.details
...
CREATE VIEW quarterview AS
	SELECT quarters.quarterid, quarters.qstart, quarters.qlatereg, quarters.qlatechange, quarters.qlastdrop,
		...
		quarters.details,
		substring(quarters.quarterid from 1 for 9)  as quarteryear, substring(quarters.quarterid from 11 for 1)  as quarter
	FROM quarters
...
CREATE VIEW qcalendarview AS
	SELECT sublevelview.degreelevelid, sublevelview.degreelevelname, sublevelview.sublevelid, sublevelview.sublevelname,
		qcalendar.qcalendarid, qcalendar.quarterid, qcalendar.qdate, qcalendar.event, qcalendar.details
...
CREATE VIEW qresidenceview AS
	SELECT residences.residenceid, residences.residencename, residences.defaultrate,
		...
		qresidences.charges, qresidences.details, qresidences.charges as residencecharge,
		qresidences.full_charges,
		quarterview.quarteryear, quarterview.quarter, quarterview.active
...
CREATE VIEW chargeview AS
	SELECT degreelevels.degreelevelid, degreelevels.degreelevelname, qcharges.qchargeid, qcharges.quarterid, 
		qcharges.studylevel, qcharges.narrative,
...
CREATE VIEW qchargeview AS
	SELECT quarters.quarterid, quarters.qstart, quarters.qlatereg, quarters.qlatechange, quarters.qlastdrop,
		...
		qcharges.qchargeid, qcharges.degreelevelid, qcharges.studylevel, qcharges.narrative,
		qcharges.fees, qcharges.fullfees, qcharges.meal2fees, qcharges.meal3fees, qcharges.premiumhall,
		qcharges.minimalfees, qcharges.firstinstalment, qcharges.firstdate,
		qcharges.secondinstalment, qcharges.seconddate
...
CREATE VIEW qmchargeview AS
	SELECT quarters.quarterid, quarters.qstart, quarters.qlatereg, quarters.qlatechange, quarters.qlastdrop,
		...
		qmcharges.qmchargeid, qmcharges.studylevel, qmcharges.charge, qmcharges.fullcharge, qmcharges.narrative
...
SELECT majors.majorid, majors.majorname, qcharges.degreelevelid, qcharges.quarterid, qcharges.studylevel,
...
CREATE OR REPLACE VIEW vwqmajorcharges AS
SELECT vwqcharges.majorid, vwqcharges.majorname, vwqcharges.degreelevelid, vwqcharges.quarterid, vwqcharges.studylevel,
...
CREATE VIEW qstudentview AS
	SELECT studentdegreeview.religionid, studentdegreeview.religionname, studentdegreeview.denominationid, studentdegreeview.denominationname,
		...
		qstudents.approved, qstudents.mealticket, qstudents.charges as extacharges
...
CREATE VIEW qstudentmajorview AS 
	SELECT studentmajorview.religionid, studentmajorview.religionname, studentmajorview.denominationid, studentmajorview.denominationname,
		studentschoolid, studentschoolname, studentmajorview.studentid, studentdepartmentid, studentdepartmentname,
...
CREATE VIEW qcourseview AS
	SELECT courseview.schoolid, courseview.schoolname, courseview.departmentid, courseview.departmentname,
		...
		qcourses.dsdate, qcourses.facultysubmit, qcourses.fsdate
...
CREATE VIEW qschoolcourseview AS
	SELECT courseview.schoolid, courseview.schoolname, courseview.departmentid, courseview.departmentname,
		...
		departments.departmentid as schooldepartmentcode
	FROM (((courseview INNER JOIN qcourses ON courseview.courseid = qcourses.courseid)
		INNER JOIN instructors ON qcourses.instructorid = instructors.instructorid)
		INNER JOIN departments ON courseview.schoolid = departments.schoolid);
...
CREATE VIEW vwgradeschoolopening AS
	SELECT courseview.schoolid, courseview.schoolname, courseview.departmentid, courseview.departmentname,
		...
		gradeopening.hoddate, gradeopening.hodid, gradeopening.regapproval, gradeopening.regreject, gradeopening.regdate,
		gradeopening.regid, gradeopening.deanapproval, gradeopening.deanreject, gradeopening.deandate, gradeopening.deanid,
		gradeopening.details, departments.departmentid as schooldepartmentcode
	FROM (((courseview INNER JOIN qcourses ON courseview.courseid = qcourses.courseid)
		INNER JOIN instructors ON qcourses.instructorid = instructors.instructorid)
		INNER JOIN gradeopening ON qcourses.qcourseid = gradeopening.qcourseid)
		INNER JOIN departments ON courseview.schoolid = departments.schoolid;
...
CREATE VIEW qetimetableview AS
	SELECT assets.assetid, assets.assetname, assets.location, assets.building, assets.capacity, 
		...
		qexamtimetable.examdate, qexamtimetable.details 
...
CREATE VIEW wvqgrades AS
	SELECT qcourseview.schoolid, qcourseview.schoolname, qcourseview.departmentid, qcourseview.departmentname,
		...
		(CASE WHEN ((qgrades.gradeid='W') OR (qgrades.gradeid='AW') OR (grades.gpacount=false) OR (qgrades.repeated=true) OR (qgrades.nongpacourse=true)) THEN 0 ELSE qgrades.credit END) as gpahours,
		(CASE WHEN ((qgrades.gradeid='W') OR (qgrades.gradeid='AW')) THEN 0 ELSE qgrades.hours END) as chargehours,
		students.studentname, students.sex, studentdegrees.studentid, qstudents.studentdegreeid, qstudents.studylevel,
		qstudents.approved
	FROM qgrades
		INNER JOIN qcourseview ON qgrades.qcourseid = qcourseview.qcourseid
		INNER JOIN grades ON qgrades.gradeid = grades.gradeid
...
CREATE VIEW vwstudentgrades AS
	SELECT studentgradeview.courseid, studentgradeview.coursetitle, studentgradeview.finalmarks, studentgradeview.credit, 
		studentgradeview.gradeid, studentgradeview.gpa, studentgradeview.quarterid, studentgradeview.studentid, 
		studentgradeview.qstudentid, studentgradeview.studentname, quarters.publishgrades
	FROM studentgradeview INNER JOIN quarters ON studentgradeview.quarterid = quarters.quarterid;
...
CREATE VIEW courseoutline (
	...
	SELECT 3, studentdegrees.studentid, studentdegrees.studentdegreeid, studentdegrees.degreeid, majors.majorname, majorcontentview.courseid,
...
CREATE VIEW corecourseoutline AS 
	SELECT 3 AS orderid, studentdegrees.studentid, studentdegrees.studentdegreeid, studentdegrees.degreeid, 
		majors.majorname AS description, majorcontentview.courseid, majorcontentview.coursetitle, majorcontentview.minor,
...
CREATE VIEW coregradeview AS 
	SELECT studentgradeview.schoolid, studentgradeview.schoolname, studentgradeview.studentid, studentgradeview.studentname, studentgradeview.sex,
		...
		studentgradeview.repeated, studentgradeview.gpahours, studentgradeview.chargehours, corecourseoutline.description, corecourseoutline.minor, corecourseoutline.elective,
		corecourseoutline.quarterdone
...
CREATE OR REPLACE VIEW vwqstudentcharges AS 
	SELECT vwstudentmajors.denominationid, vwstudentmajors.denominationname, vwstudentmajors.studentid, vwstudentmajors.studentname, 
		...
		quarters.active, qresidenceview.residenceid, qresidenceview.residencename, 
	...
	FROM vwstudentmajors
		JOIN (qstudents
		JOIN quarters ON qstudents.quarterid::text = quarters.quarterid::text) ON vwstudentmajors.studentdegreeid = qstudents.studentdegreeid
		JOIN qresidenceview ON qstudents.qresidenceid = qresidenceview.qresidenceid
		JOIN qcharges ON vwstudentmajors.degreelevelid::text = qcharges.degreelevelid::text AND qstudents.quarterid::text = qcharges.quarterid::text AND qstudents.studylevel = qcharges.studylevel
		LEFT JOIN qmcharges ON vwstudentmajors.majorid::text = qmcharges.majorid::text AND qstudents.quarterid::text = qmcharges.quarterid::text AND qstudents.studylevel = qmcharges.studylevel;
...
CREATE VIEW vwscholarships AS
	SELECT students.studentid, students.studentname, students.accountnumber, students.Nationality, students.Sex,
		...
		scholarships.quarterid, scholarships.scholarshipid, scholarships.entrydate, scholarships.paymentdate,
...
CREATE VIEW smscholarships AS
	SELECT scholarships.studentid, scholarships.quarterid, sum(scholarships.amount) as scholarship
	FROM scholarships
	WHERE (scholarships.approved = true)
	GROUP BY scholarships.studentid, scholarships.quarterid;
...
CREATE VIEW vwstudentpayments AS
	SELECT students.studentid, students.studentname, students.accountnumber,
		qstudents.qstudentid, qstudents.quarterid, qstudents.financeclosed, studentpayments.studentpaymentid,
...
CREATE VIEW vwqstudentbalances AS 
	SELECT vwqstudentcharges.studentid, vwqstudentcharges.studentname, vwqstudentcharges.nationality, 
		...
		vwqstudentcharges.residenceid, vwqstudentcharges.residencename, 
...
CREATE VIEW vwqchargedefinations AS
	SELECT chargetypes.chargetypeid, chargetypes.chargetypename, chargetypes.accountnumber, chargetypes.accountcode,
		qchargedefinations.qchargedefinationid,
...
CREATE VIEW vwqmchargedefinations AS
	SELECT chargetypes.chargetypeid, chargetypes.chargetypename, chargetypes.accountnumber, chargetypes.accountcode,
		chargetypes.oncampus, chargetypes.offcampus,
		majors.majorid, majors.majorname, qmchargedefinations.qmchargedefinationid, qmchargedefinations.quarterid,
...
CREATE VIEW vwqmajorchargelists AS
	(SELECT majors.majorid, majors.majorname, vwqchargedefinations.chargetypeid, vwqchargedefinations.chargetypename,
	...
	(SELECT vwqmchargedefinations.majorid, vwqmchargedefinations.majorname,
		vwqmchargedefinations.chargetypeid, vwqmchargedefinations.chargetypename, vwqmchargedefinations.accountnumber,
		vwqmchargedefinations.accountcode, vwqmchargedefinations.qmchargedefinationid,
		vwqmchargedefinations.quarterid, vwqmchargedefinations.studylevel, vwqmchargedefinations.narrative,
...
CREATE VIEW vwqmajorchargesummary AS
	SELECT vwqmajorchargelists.quarterid, vwqmajorchargelists.majorid, vwqmajorchargelists.majorname, vwqmajorchargelists.studylevel, 
	...
	GROUP BY vwqmajorchargelists.quarterid, vwqmajorchargelists.majorid, vwqmajorchargelists.majorname, vwqmajorchargelists.studylevel;
...
CREATE VIEW vwsuncharges AS 
	SELECT vwqmajorchargelists.accountnumber AS chargeaccount, vwqmajorchargelists.accountcode, 
		...
		vwqstudentbalances.accountnumber, vwqstudentbalances.studentname, vwqstudentbalances.fees, vwqstudentbalances.picked, 
		vwqstudentbalances.qstudentid
