...
function fillDropDownValuesFromXml() {
	...
	try {
		...
		for(i=0; i<xmlNodeList.length ;i++) {
			...
			else if(nodeName=='CustomerHierarchyLevel')
				dropDownName = frmCwtCorpScrpt.cboCustomerHierarchy;
			...
			else if(nodeName=='cboRegionClass')
				dropDownName = frmCwtCorpScrpt.cboRegionClass;
...
function ExecuteScript(checkbug) {
		allCommands = '';
		allErrors = null;
		...
		var hierachy = frmCwtCorpScrpt.txtHierachyID.value;
		var custhierachy = frmCwtCorpScrpt.cboCustomerHierarchy.value
		if((custhierachy != '') && (hierachy != ''))
			addCommand('DI.FT-' + custhierachy, hierachy);
			
		if(frmCwtCorpScrpt.cboPredominantClass.value!='')
			addCommand('DI.FT-FF8/', frmCwtCorpScrpt.cboPredominantClass.value);

		if(frmCwtCorpScrpt.cboRegionClass.value!='')
			addCommand('DI.FT-RC/', frmCwtCorpScrpt.cboRegionClass.value);
		...
		if(frmCwtCorpScrpt.cboSupplierCode.value !='')
			...
			alert('Select an option for the supplier code...');
