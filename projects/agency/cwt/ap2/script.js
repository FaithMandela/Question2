...
function fillDropDownValuesFromXml() {
	...
	try {
		...
		for(i=0; i<xmlNodeList.length ;i++) {
			...
			else if(nodeName=='TicketNumber')				dropDownName = frmCwtCorpScrpt.cboTicketNumber;			else if(nodeName=='PredominantClass')
...
function addCommand(strCommand, strFunction) {
	if(frmCwtCorpScrpt.cboTicketNumber.value=='') {
		allCommands += concatChar + strCommand + strFunction;
	} else if (frmCwtCorpScrpt.cboTicketNumber.value=='1') {
		allCommands += concatChar + strCommand + strFunction;
	} else {
		allCommands += concatChar + strCommand + '*' + frmCwtCorpScrpt.cboTicketNumber.value + '/' + strFunction;
	}
}
...
function ExecuteScript() {
		...
		if(frmCwtCorpScrpt.txtHierachyID2.value!='')
			addCommand('DI.FT-FF2/', frmCwtCorpScrpt.txtHierachyID2.value);

		if(frmCwtCorpScrpt.txtHierachyID3.value!='')
			addCommand('DI.FT-FF3/', frmCwtCorpScrpt.txtHierachyID3.value);

		if(frmCwtCorpScrpt.txtHierachyID4.value!='')
			addCommand('DI.FT-FF4/', frmCwtCorpScrpt.txtHierachyID4.value);

		if(frmCwtCorpScrpt.txtHierachyID5.value!='')
			addCommand('DI.FT-FF5/', frmCwtCorpScrpt.txtHierachyID5.value);

		if(frmCwtCorpScrpt.txtTravellerID.value!='')
			addCommand('DI.FT-FF25/', frmCwtCorpScrpt.txtTravellerID.value);
		...
		if(frmCwtCorpScrpt.txtBaseSellingFare.value!='')
			addCommand('DI.FT-SF/', frmCwtCorpScrpt.txtBaseSellingFare.value);
		
		if(frmCwtCorpScrpt.cboRealisedSavingCode.value!='')
			addCommand('DI.FT-FF30/', frmCwtCorpScrpt.cboRealisedSavingCode.value);
		
		if(frmCwtCorpScrpt.txtLpoAuthorization.value!='')
			addCommand('DI.FT-FF10/', frmCwtCorpScrpt.txtLpoAuthorization.value );
		...
		if(frmCwtCorpScrpt.cboSupplierCode.value !='')
			addCommand('DI.FT-FF47/', frmCwtCorpScrpt.cboSupplierCode.value);
		...
		addCommand('DI.FT-FF36/', 'G');
		addCommand('DI.FT-FF35/', 'GAL');
		addCommand('DI.FT-FF34/', 'AB');
