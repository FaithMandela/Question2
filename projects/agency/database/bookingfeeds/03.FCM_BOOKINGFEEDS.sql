...
SELECT @v_FlightList = 
	COALESCE (CASE WHEN @v_FlightList = ''
				...
				END ,' ')
	FROM [GIDS_FCM].[dbo].[AirTktSeg]  WHERE[TravelOrderIdentifier] = @TravelOrderIdentifier;
	--PRINT @v_FlightList
...
CREATE FUNCTION GIDS_FCM.dbo.GetPassangerName(@TravelOrderIdentifier as int) RETURNS varchar(MAX) AS
BEGIN
DECLARE 	@v_PaxName 				VarChar(MAX) = '' ;
	SELECT @v_PaxName = [LastName] + ' ' + [FirstName]  FROM [GIDS_FCM].[dbo].[Passenger]  WHERE [TravelOrderIdentifier] = @TravelOrderIdentifier;
	--PRINT @v_PaxName
...
CREATE VIEW vw_sms AS
	SELECT [sms_id], [TravelOrderIdentifier], [RecordLocator],[HostEventTimeStamp], [PhoneNbr],  [is_sent],
		[GIDS_FCM].[dbo].GetPassangerName([TravelOrderIdentifier]) + CHAR(10) + [TicketNumber] + [GIDS_FCM].[dbo].GetFlights([TravelOrderIdentifier], CHAR(10) ) AS message
	FROM [GIDS_FCM].[dbo].[sms];

	
	
  CREATE VIEW [dbo].[vw_email] AS
	SELECT [email_id], [TravelOrderIdentifier], [pcc],[son], [PhoneNbr], [GIDS_FCM].[dbo].GetPassangerName([TravelOrderIdentifier]) AS [PassangerName], [RecordLocator], [is_picked],[HostEventTimeStamp], 
		'PCC : ' + [pcc] 
		...
		+ CHAR(13) + CHAR(10) + '<br>'
		+ CHAR(13) + CHAR(10) + '<br>' + [GIDS_FCM].[dbo].GetPassangerName([TravelOrderIdentifier])
		+ CHAR(13) + CHAR(10) + '<br>' + [GIDS_FCM].[dbo].GetFlights([TravelOrderIdentifier], (CHAR(13) + CHAR(10) + '<br>')) AS message
	FROM [GIDS_FCM].[dbo].[email];
	
	
CREATE TRIGGER [dbo].[ins_TktTrans] ON [GIDS_FCM].[dbo].[TktTrans] AFTER INSERT AS
BEGIN
	...
	BEGIN
		...
		DECLARE @v_Message					varchar(255),	@v_TicketNumber 			varchar(50);
			...
			SELECT @v_RecordLocator = [RecordLocator],
					...
					@v_TransactionAgent = [TransactionAgent]  FROM [GIDS_FCM].[dbo].[TravelOrderEvent] WHERE [TravelOrderIdentifier] = (SELECT TravelOrderIdentifier FROM INSERTED);
					
			SELECT @v_PassngerName =  [GIDS_FCM].[dbo].GetPassangerName(@v_TTravelOrderIdentifier);
			...
			SELECT @v_FlightList = [GIDS_FCM].[dbo].GetFlights(@v_TTravelOrderIdentifier, (CHAR(13) + CHAR(10) + '<br>'));
			
			SELECT @v_Message =  [GIDS_FCM].[dbo].GetFlights(@v_TTravelOrderIdentifier, (CHAR(13) + CHAR(10) + '<br>'));
