...
CREATE TRIGGER ins_TktTrans ON [GIDS].[dbo].[TktTrans] AFTER INSERT AS
BEGIN
	...
	BEGIN
		...
		DECLARE @v_Message					varchar(255),	@v_TicketNumber 			varchar(50);
			...
			SELECT @v_RecordLocator = [RecordLocator],
					...
					@v_TransactionAgent = [TransactionAgent]  FROM [GIDS].[dbo].[TravelOrderEvent] WHERE [TravelOrderIdentifier] = (SELECT TravelOrderIdentifier FROM INSERTED);
					
			SELECT @v_PassngerName =  [GIDS].[dbo].GetPassangerName(@v_TTravelOrderIdentifier);
			...
			SELECT @v_FlightList = [GIDS].[dbo].GetFlights(@v_TTravelOrderIdentifier, (CHAR(13) + CHAR(10) + '<br>'));
			
			SELECT @v_Message =  [GIDS].[dbo].GetFlights(@v_TTravelOrderIdentifier, (CHAR(13) + CHAR(10) + '<br>'));
			...
			IF((LTRIM(RTRIM(@v_PhoneNbr))) IS NOT NULL  ) OR (LEN((LTRIM(RTRIM(@v_PhoneNbr)))) > 0)
				BEGIN
					INSERT INTO dbo.sms(sms_id ,TravelOrderIdentifier,pcc,son,is_sent,RecordLocator, PhoneNbr, PassangerName, message, HostEventTimeStamp, TicketNumber)
						(SELECT (NEXT VALUE FOR seq_sms_id), @v_TTravelOrderIdentifier, @v_OwningAgencyPseudo , @v_TransactionAgent, 0 ,@v_RecordLocator, @v_PhoneNbr, @v_PassngerName , @v_Message, @v_THostEventTimeStamp, @v_TicketNumber  );
				END
			...
			IF (@v_THostEventTimeStamp >= CAST((CAST((CAST(@v_THostEventTimeStamp AS DATE)) AS VARCHAR) + ' 18:00:000') AS DATETIME))	AND (@v_THostEventTimeStamp  <= CAST((CAST((CAST((@v_THostEventTimeStamp) AS DATE)) AS VARCHAR) + ' 08:00:000') AS DATETIME) + 1)
				BEGIN
					INSERT INTO dbo.email(email_id ,TravelOrderIdentifier,pcc,son,is_picked,RecordLocator, PhoneNbr, PassangerName, message, HostEventTimeStamp)
						(SELECT (NEXT VALUE FOR seq_email_id), @v_TTravelOrderIdentifier, @v_OwningAgencyPseudo , @v_TransactionAgent, 0 ,@v_RecordLocator, @v_PhoneNbr, @v_PassngerName , @v_Message, @v_THostEventTimeStamp );
				END
	END
END
...
SELECT @v_FlightList = 
	COALESCE (CASE WHEN @v_FlightList = ''
				...
				END ,' ')
	FROM [GIDS].[dbo].[AirTktSeg]  WHERE[TravelOrderIdentifier] = @TravelOrderIdentifier;
	--PRINT @v_FlightList
...
CREATE FUNCTION GIDS.dbo.GetPassangerName(@TravelOrderIdentifier as int) RETURNS varchar(MAX) AS
BEGIN
DECLARE 	@v_PaxName 				VarChar(MAX) = '' ;
	SELECT @v_PaxName = [LastName] + ' ' + [FirstName]  FROM [GIDS].[dbo].[Passenger]  WHERE [TravelOrderIdentifier] = @TravelOrderIdentifier;
	--PRINT @v_PaxName
...
CREATE VIEW vw_sms AS
	SELECT [sms_id], [TravelOrderIdentifier], [RecordLocator],[HostEventTimeStamp], [PhoneNbr],  [is_sent],
		[GIDS].[dbo].GetPassangerName([TravelOrderIdentifier]) + CHAR(10) + [TicketNumber] + [GIDS].[dbo].GetFlights([TravelOrderIdentifier], CHAR(10) ) AS message
	FROM [GIDS].[dbo].[sms];

	
	
  CREATE VIEW vw_email AS
	SELECT [email_id], [PhoneNbr],[is_picked],[HostEventTimeStamp], 
		'PCC : ' + [pcc] 
		...
		+ CHAR(13) + CHAR(10) + '<br>'
		+ CHAR(13) + CHAR(10) + '<br>' + [GIDS].[dbo].GetPassangerName([TravelOrderIdentifier])
		+ CHAR(13) + CHAR(10) + '<br>' + [GIDS].[dbo].GetFlights([TravelOrderIdentifier], (CHAR(13) + CHAR(10) + '<br>')) AS message
	FROM [GIDS].[dbo].[email];
	-- last day pick email_id = 230
	
	
	
	/****** Script for SelectTopNRows command from SSMS  ******/
SELECT TOP 1000 
		[TravelOrderEvent].[HostEventTimeStamp],
		[TravelOrderEvent].[RecordLocator],
		GIDS.dbo.GetPassangerName(t.[TravelOrderIdentifier]) AS Name
	   ,t.[TravelOrderIdentifier]
      ,t.[TicketNbr],t.[TransactionYear] ,t.[TransactionCrs]
      ,t.[TransactionStockType]
      ,t.[TicketType]
      ,t.[ActiveVoidStatus]
      ,t.[TransactionPlatingAlpha]
      ,t.[TransactionPlatingNbr]
      ,t.[TransactionType]
      ,t.[CurrencyCode]
      ,t.[DecimalNbr]
      ,t.[OBFTotalAmount]
      ,t.[OBFGrandTotalAmount]
      ,t.[TourCode]
      ,t.[ITBTIndicator]
      ,t.[NetRemitIndicator]
      ,t.[FareCalculationPricingInd]
      ,t.[InvoiceNumber]
      ,t.[AirlineCommissionPercent]
      ,t.[AirlineCommissionAmount]
  FROM [GIDS].[dbo].[TktTrans] AS t
  INNER JOIN [GIDS].[dbo].[TravelOrderEvent] ON [TravelOrderEvent].[TravelOrderIdentifier] = t.[TravelOrderIdentifier]
 -- WHERE [ActiveVoidStatus] = 'A' 
  ORDER BY [TravelOrderIdentifier] DESC