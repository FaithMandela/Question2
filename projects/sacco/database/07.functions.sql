...
CREATE OR REPLACE FUNCTION get_balance_interest_penalty ( entity_id integer)   RETURNS real AS
  $$
DECLARE
...
CREATE OR REPLACE VIEW vw_contributions AS 
 SELECT contributions.contribution_id,
    ...
    get_balance_interest_penalty(contributions.entity_id) AS active_balance,
...
BEGIN
	...
	New.receipt_date = now()::date;
		...
		SELECT sum(contributions.receipt - contributions.contribution_paid) INTO v_contributions
		FROM contributions INNER JOIN periods ON contributions.period_id = periods.period_id
		...
		SELECT sum(contribution_amount - contribution_paid) INTO v_contribution
		FROM contributions INNER JOIN periods ON contributions.period_id = periods.period_id
		WHERE (contributions.contribution_id <> NEW.contribution_id) AND (contributions.entity_id = NEW.entity_id)
			AND (periods.end_date <= NEW.entry_date);
		IF(v_contributions is null)THEN v_contributions = 0; END IF;
