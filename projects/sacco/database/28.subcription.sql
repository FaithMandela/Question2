...
ALTER TABLE orgs ADD member_limit integer default 50 not null;
...
BEGIN
	...
	ELSIF(NEW.approve_status = 'Approved')THEN
		...
		INSERT INTO investment_types (org_id, investment_type_name, interest_amount) VALUES (NEW.org_id, 'Land', 10);

		INSERT INTO investment_status (org_id, investment_status_name) VALUES 
		(NEW.org_id, 'Proposal'), (NEW.org_id, 'Rejected'), (NEW.org_id, 'Commited'), (NEW.org_id, 'Executed');
		...
		(v_member_id, v_product_id, NEW.org_id, true, 'Approved', 'Loan', -100000000000);
...
BEGIN
	...
	IF(v_member_count > v_member_limit)THEN
		RAISE EXCEPTION 'You have reached the maximum Members limit, request for a quite for more';
	END IF;
