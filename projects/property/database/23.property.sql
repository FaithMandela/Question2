...
CREATE OR REPLACE FUNCTION get_occupied(integer) RETURNS integer AS $$
	...
	FROM units
	WHERE (is_vacant = false) AND (property_id = $1);
$$ LANGUAGE SQL;
...
CREATE OR REPLACE VIEW vw_tenants AS
	SELECT 
		entitys.entity_id, tenants.tenant_id, tenants.person_title, tenants.tenant_name, 
...
CREATE OR REPLACE VIEW vw_landlord AS
	...
	landlord.person_title, landlord.landlord_name, landlord.identification_number, 
...
CREATE OR REPLACE VIEW vw_property AS
	SELECT landlord.landlord_id as client_id, landlord.landlord_name as client_name, 
...
CREATE OR REPLACE VIEW vw_rentals AS
	SELECT vw_property.client_id, vw_property.client_name, vw_property.property_type_id, 
...
CREATE OR REPLACE VIEW vw_tenant_rentals AS
	SELECT tenants.tenant_id, tenants.tenant_name,
